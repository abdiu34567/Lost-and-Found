...
function EscapePhoto(id) {
  const data = Api().catches;
  var catched = data.get(id);
  var obj = JSON.parse(catched);

  if (obj['State'] != 'photo') return Bot.sendText(id, Message().Welcome, Inline().setup);

  Bot.sendText(id, NoPhoto().PhotoSave, Reply().phoneshare);
  obj['State'] = 'phone';
  obj = JSON.stringify(obj);
  data.put(id, obj, 1200)//catch expire after 20 minute
  return null;
}

function CatchStatus(id, dota) {
  const data = Api().catches.get(id);

  if (data != null) var obj = JSON.parse(data); // if catch available
